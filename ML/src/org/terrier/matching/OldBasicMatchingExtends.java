...
import org.terrier.matching.dsms.IntegrateStaticScore;
...
/**
 * <i>This is the original matching implementation of Terrier</i>. Performs the
 * matching of documents with a query, by first assigning scores to documents
 * for each query term and modifying these scores with the appropriate
 * modifiers. Then, a series of document score modifiers are applied if
 * necessary.
 *
 * @author Vassilis Plachouras, Craig Macdonald
 */
public class OldBasicMatchingExtends implements Matching {
    ...
    /**
     * The default namespace for the document score modifiers that are specified
     * in the properties file.
     */
    protected static final String dsmNamespace = "org.terrier.matching.dsms.";

    /**
     * The maximum number of documents in the final retrieved set. It
     * corresponds to the property <tt>matching.retrieved_set_size</tt>. The
     * default value is 1000, however, setting the property to 0 will return all
     * matched documents.
     */
    protected static int RETRIEVED_SET_SIZE;

    /**
     * A property that enables to ignore the terms with a low IDF. In the match
     * method, we check whether the frequency of a term in the collection is
     * higher than the number of documents. If this is true, then by default we
     * don't assign scores to documents that contain this term. We can change
     * this default behaviour by altering the corresponding property
     * <tt>ignore.low.idf.terms</tt>, the default value of which is true.
     */
    protected static boolean IGNORE_LOW_IDF_TERMS;

    /**
     * A property that when it is true, it allows matching all documents to an
     * empty query. In this case the ordering of documents is random. More
     * specifically, it is the ordering of documents in the document index. The
     * corresponding property is
    ...
    //declarer meta  
       public static Index index1 = Index.createIndex();
       public static MetaIndex meta = index1.getMetaIndex();
    ...
    protected OldBasicMatchingExtends() {
    ...
    /**
     * A default constructor that creates the CollectionResultSet and
     * initialises the document and term modifier containers.
     *
     * @param _index the object that encapsulates the basic data structures used
     * for retrieval.
     */
    public OldBasicMatchingExtends(Index _index) {
        
        documentModifiers = new ArrayList<DocumentScoreModifier>();
        IntegrateStaticScore priorClasse = new IntegrateStaticScore();
        ...
        meta = index1.getMetaIndex();
        Properties prop = new Properties();
        FileInputStream input = null;
        int docid = 0;
        try {
            System.out.println("je suis dans le matching");
            input = new FileInputStream("C:/poiexcel/data.properties");
            if (input == null) {
            ...
            while (e.hasMoreElements()) {
                String docno = (String) e.nextElement();
                String value = prop.getProperty(docno);
                Double valeur = Double.parseDouble(value);
                map.put(docno, valeur);
        ...
        /*for (Map.Entry<String, Double> map1 : map.entrySet()) {
            String myKey = map1.getKey();
            Double monScore = map1.getValue();
        ...
        String defaultDSMS = ApplicationSetup.getProperty("matching.dsms", "IntegrateStaticScore");
...
//			}
    ...
    /**
     * Initialises the arrays prior of retrieval. Only the first time it is
     * called, it will allocate memory for the arrays.
     */
    ...
    /**
     * Initialises the arrays prior of retrieval, with the given scores. Only
     * the first time it is called, it will allocate memory for the arrays.
     *
    ...
    /**
     * Registers a document score modifier. If more than one modifiers are
     * registered, then they applied in the order they were registered.
     *
     * @param documentScoreModifier DocumentScoreModifier the score modifier to
     * be applied.
     */
...
//populated one for each document in collection
        ...
        public boolean modifyScores(Index I,
            ...
            for (int i = 0; i < scores.length; i++) {
                try {
                    String doc = meta.getItem("docno", docids[i]);
                    scores[i] = scores[i] + map.get(doc);
